...
```shell
/bin/bash -c "$(curl -fsSL https://github.com/ernestojballon/gitbash-in-mac/blob/master/install.sh)"
```
...
1. from the `Terminal`, run
```shell
/bin/bash -c "$(curl -fsSL https://github.com/ernestojballon/gitbash-in-mac/blob/master/uninstall.sh)"
```
